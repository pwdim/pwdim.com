  ...
  const Perfil = () => {
    ...
    const [expandedStats, setExpandedStats] = useState(null); // Changed to store only one key
    ...
    const rank_colors = {
      ...
      "FLAME": [255, 170, 0],
      "SPARK": [255, 255, 85],
      "Membro": [221, 221, 221] // Cor padrão para Membro
    };
    ...
    const fetchPlayerData = async (nick) => {
      ...
      setLoading(true);
      setVisibleStats({});
      setExpandedStats(null); // Reset expanded stats on new player fetch
    ...
    const getRankColor = (rank) => {
      ...
      return '#ddd'; // Cor padrão em hexadecimal
    };
    ...
    const toggleExpand = (modeKey) => {
      setExpandedStats(expandedStats === modeKey ? null : modeKey); // Toggle visibility
    };
    ...
    const gameModesConfigWithImages = {
      ...
      bridge: { ...gameModesConfig.bridge, backgroundImage: 'url("https://imgur.com/JgXQdID.png")' },
    };

    const categorizedGameModes = {
      PVP: ["arena", "fps"],
      DUELS: ["gladiator", "simulator", "sopa", "bridge"],
      HG: ["hg", "league", "competitive"],
      BEDWARS: ["bedwars_geral", "bedwars_solo", "bedwars_dupla", "bedwars_trio", "bedwars_quarteto"],
    };

    const [activeCategory, setActiveCategory] = useState("PVP"); // Define PVP como a categoria inicial
          ...
          <S.GeneralInfoSection>
            <S.GeneralInfoContainer>
              ...
              {playerData?.playerRanks && playerData.playerRanks.length > 0 && (
                <S.InfoCard className="rank-info">
                  <S.InfoTitle>Rank:</S.InfoTitle>
                  <S.InfoValue>
                    <S.RankBadge $backgroundColor={getRankColor(playerRank)}>{playerRank}</S.RankBadge> {/* Use $backgroundColor */}
                  </S.InfoValue>
                </S.InfoCard>
              )}
              {playerData?.clan && (
                <S.InfoCard className="clan-info">
              ...
              {playerData?.premium !== undefined && (
                <S.InfoCard className="account-info">
            ...
            <S.MinigamesGrid>
              {categorizedGameModes[activeCategory]?.map((modeKey) => {
                ...
                if (modeKey.startsWith("bedwars_")) {
                  ...
                  const bwTitle = `BedWars (${bwMode.charAt(0).toUpperCase() + bwMode.slice(1)})`; // Format title
                        ...
                        {Object.entries(bwStatsConfig).map(([statKey, statName]) => {
                          ...
                          return (
                            <S.StatItem key={statKey}>
                              <S.StatName dangerouslySetInnerHTML={{ __html: statName }} /> {/* Render HTML-like icons */}
                    ...
                    <S.MinigameStats key={modeKey} $backgroundImage={modeInfo.backgroundImage}>
                      ...
                      <S.StatsWrapper>
                        ...
                        {modeKey === 'league' && playerData.leaguePosition !== null && (
                          <S.StatItem key="league_position">
                            <S.StatName>Colocação:</S.StatName>
                            <S.StatValue>{playerData.leaguePosition}</S.StatValue>
                          </S.StatItem>
                        )}
                      </S.StatsWrapper>
                    </S.MinigameStats>
